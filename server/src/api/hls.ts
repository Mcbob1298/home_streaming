...
async function resolve(
  ...
  return resolvePlayback(
    ...
    { transcodeAvailable: true, capacites },
  );
}
...
export function registerHlsRoutes(app: FastifyInstance, db: Db, sessions: () => SessionManager | null): void {
  ...
  async function context(
     ...
     */
    const capacites = capacitesDe(request.headers);

    return { manager, media, resolved: await resolve(db, manager, media, capacites), id, capacites };
  }

  /** La session du fichier, créée au besoin avec tout ce qu'il faut produire. */
  async function acquire(manager: SessionManager, media: MediaRow, resolved: ResolvedPlayback, capacites: CapacitesClient) {
    ...
    const hdrPassthrough = hdrPassthroughFor({
      clientDecodesHevc: capacites.hevc,
  ...
  app.get('/api/hls/:id/index.m3u8', async (request, reply) => {
    ...
    const sortie = outputGeometry({
      ...
      hdrPassthrough: hdrPassthroughFor({ clientDecodesHevc: found.capacites.hevc, source: resolved.source, mode }),
    });
